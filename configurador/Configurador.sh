...
dovecot_configurar_dovecot_conf() {
	...
	python3 sustituir_texto.py -s /etc/dovecot/dovecot.conf${renombrado} -o /etc/dovecot/dovecot.conf -p1 '#listen = *, ::' -t1 'listen = *, ::'
...
dovecot_configurar_10_auth_conf() {
        ...
        python3 sustituir_texto.py -s /etc/dovecot/conf.d/10-auth.conf${renombrado} -o /etc/dovecot/conf.d/10-auth.conf -p1 '#disable_plaintext_auth = yes' -t1 'disable_plaintext_auth = no' -p2 'auth_mechanisms = plain' -t2 'auth_mechanisms = plain login'
}
dovecot_configurar_10_mail_conf() {
        ...
        python3 sustituir_texto.py -s /etc/dovecot/conf.d/10-mail.conf${renombrado} -o /etc/dovecot/conf.d/10-mail.conf -p1 'mail_location = mbox:~/mail:INBOX=/var/mail/%u' -t1 'mail_location = maildir:~/Maildir'
}
dovecot_configurar_10_master_conf() {
	...
	python3 sustituir_texto.py -s /etc/dovecot/conf.d/10-master.conf${renombrado} -o /etc/dovecot/conf.d/10-master.conf -p1 '  #unix_listener /var/spool/postfix/private/auth {' -t1 "$lineas"
}
cambiar_a_Maildir() {
	if [[ $(python3 existe_texto.py -s /etc/profile -p 'export MAIL=$HOME/Maildir') = 0 ]]; then 
...
fetchmail_configurar_etc_default_fetchmail() {
        ...
        python3 sustituir_texto.py -s /etc/default/fetchmail${renombrado} -o /etc/default/fetchmail -p1 'START_DAEMON=no' -t1 'START_DAEMON=yes '
...
paquete="rsyslog" #En Debian 12 este paquete no esta ya por defecto
instalar_sino_esta
paquete="postfix"
instalar_sino_esta
paquete="fetchmail"
